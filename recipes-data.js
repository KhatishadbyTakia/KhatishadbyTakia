const recipes = [

{
    id: "khichuri",

    title: "ঘি দিয়ে খিচুড়ি",

    category: "দেশি ঘি",

    image: "images/ghe 3.png",

    video: "https://www.facebook.com/reel/936316012087791/?mibextid=rS40aB7S9Ucbxw6v",

    description:
    "খাঁটি দেশি ঘির সুগন্ধে তৈরি নরম ও সুস্বাদু খিচুড়ি।",

    rating: "5.0",

    time: "৩০ মিনিট",

    serves: "৪ জন",

    difficulty: "সহজ",

    productName: "দেশি গাওয়া ঘি",

    productImage: "images/ghe 3.png",

    productLink:
    "product-details.html?id=ghee",


    ingredients:[

        "২ কাপ চাল",
        "১ কাপ মুগ ডাল",
        "৩ টেবিল চামচ দেশি ঘি",
        "১টি পেঁয়াজ কুচি",
        "১ চা চামচ আদা বাটা",
        "১ চা চামচ রসুন বাটা",
        "হলুদ গুঁড়া",
        "লবণ",
        "গরম মসলা",
        "পরিমাণমতো পানি"

    ],


    steps:[

        {
            title:"ধাপ ১",
            text:"মুগ ডাল হালকা ভেজে ধুয়ে নিন।"
        },

        {
            title:"ধাপ ২",
            text:"চাল ধুয়ে ডালের সাথে মিশিয়ে নিন।"
        },

        {
            title:"ধাপ ৩",
            text:"ঘি গরম করে পেঁয়াজ, আদা ও রসুন ভেজে নিন।"
        },

        {
            title:"ধাপ ৪",
            text:"সব উপকরণ দিয়ে পানি দিয়ে রান্না করুন।"
        }

    ]

},



{
    id:"honey-drink",

    title:"মধু দিয়ে লেবু চা",

    category:"খাঁটি মধু",

    image:"images/modu-4.jpeg",

    video:"https://www.facebook.com/share/r/1HegmB3Hbz/",

    description:
    "খাঁটি মধু ও লেবুর সতেজতায় তৈরি স্বাস্থ্যকর পানীয়।",

    rating:"4.9",

    time:"১০ মিনিট",

    serves:"২ জন",

    difficulty:"সহজ",

    productName:"খাঁটি মধু",

    productImage:"images/modu-4.jpeg",

    productLink:
    "product-details.html?id=honey",


    ingredients:[

        "২ কাপ গরম পানি",
        "২ চা চামচ খাঁটি মধু",
        "১টি লেবুর রস",
        "চা পাতা",
        "আদা"

    ],


    steps:[

        {
            title:"ধাপ ১",
            text:"চা পাতা দিয়ে পানি ফুটিয়ে নিন।"
        },

        {
            title:"ধাপ ২",
            text:"লেবুর রস ও মধু মিশিয়ে নিন।"
        }

    ]

},



{
    id:"mustard-fish",

    title:"সরিষার তেলে বেগুন ভর্তা",

    category:"সরিষার তেল",

    image:"images/sorisa-3.jpeg",

    video:"https://www.facebook.com/share/r/1EGCBfx1Bp/",

    description:
    "খাঁটি সরিষার তেলের ঘ্রাণে তৈরি গ্রামবাংলার ঐতিহ্যবাহী বেগুন ভর্তা।",

    rating:"5.0",

    time:"২৫ মিনিট",

    serves:"৩ জন",

    difficulty:"সহজ",


    productName:"খাঁটি সরিষার তেল",

    productImage:"images/sorisa-3.jpeg",

    productLink:
    "product-details.html?id=mustard-oil",


    ingredients:[

        "১টি বড় বেগুন",
        "২ টেবিল চামচ সরিষার তেল",
        "পেঁয়াজ কুচি",
        "কাঁচা মরিচ",
        "ধনেপাতা",
        "লবণ"

    ],


    steps:[

        {
            title:"ধাপ ১",
            text:"বেগুন পুড়িয়ে নরম করে নিন।"
        },

        {
            title:"ধাপ ২",
            text:"সব উপকরণ সরিষার তেল দিয়ে মিশিয়ে নিন।"
        }

    ]

},


{
   
    id: "rosunacar",

    title: "রসুনের আচার",

    category: "আচার",

    image: "images/rosun-achar.jpeg",

    video: "videos/rosun-acar.mp4",

    description: "ঘরোয়া মসলায় তৈরি ঝাল-মিষ্টি স্বাদের রসুনের আচার।",

    rating: "4.9",

    time: "৪০ মিনিট",

    serves: "১০ জন",

    difficulty: "মাঝারি",

    productName: "রসুনের আচার",

    productImage: "images/rosun-achar.jpeg",

    productLink: "product-details.html?id=garlic-pickle",

    ingredients: [

        "৫০০ গ্রাম রসুন",

        "সরিষার তেল",

        "মরিচ গুঁড়া",

        "হলুদ গুঁড়া",

        "লবণ",

        "ভিনেগার"

    ],

    steps: [

        {
            title: "ধাপ ১",
            text: "রসুন পরিষ্কার করে ভালোভাবে ধুয়ে শুকিয়ে নিন।"
        },

        {
            title: "ধাপ ২",
            text: "সরিষার তেল গরম করে ঠান্ডা করুন এবং মসলা মিশিয়ে নিন।"
        },

        {
            title: "ধাপ ৩",
            text: "রসুনের সঙ্গে সব মসলা ভালোভাবে মিশিয়ে কাচের বোতলে ভরে রাখুন।"
        },

        {
            title: "ধাপ ৪",
            text: "৩–৫ দিন রোদে রাখার পর পরিবেশনের জন্য প্রস্তুত।"
        }

    ]

},

];
